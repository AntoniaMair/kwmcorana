...
export class VaceventService {
  ...
  getAll(location_id: number): Observable<Array<Vacevent>> {
    return this.http
      .get(`${this.api}/vacevent/searchloc/${location_id}`)
      .pipe(retry(3))
      .pipe(catchError(this.errorHandler));
  }
  getSingle(id: number): Observable<Vacevent> {
    return this.http
      .get<Vacevent>(`${this.api}/vacevent/${id}`)
      .pipe(retry(3))
      .pipe(catchError(this.errorHandler));
  }
  getByVaccinated(vaccinated_id: number): Observable<Vacevent> {
    return this.http
      .get<Vacevent>(`${this.api}/vacevent/search/${vaccinated_id}`)
      .pipe(retry(3))
      .pipe(catchError(this.errorHandler));
  }
  create(vacevent: Vacevent): Observable<any> {
    return this.http
      .post(`${this.api}/vacevent`, vacevent)
      .pipe(retry(3))
      .pipe(catchError(this.errorHandler));
  }
